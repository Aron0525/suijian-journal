...
- 每次登录、回到页面或网络恢复时会拉取云端内容，再按 `updatedAt` 合并；删除会作为同步标记保留，另一台设备也会删除同一条内容。输入框中的草稿也会按“账号 + 日期”同步；同一日期有跨设备编辑时以最新更新时间为准，旧副本不会静默覆盖新副本。若线上尚未建立 `journal_drafts` 表，应用会使用现有日记表中的账号私有隐藏兼容记录同步草稿；该记录不会显示在日历或档案中，数据库迁移完成后会优先使用专用草稿表。若旧库还缺少附件字段或私有附件桶，已保存日记的标签、心情、工作内容与受限大小的附件会暂存于同一条日记的兼容数据中并随账号同步；结构升级后，新保存内容会恢复使用专用字段和 Storage。
