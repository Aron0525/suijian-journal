...
- 云端保存日记（包含已保存的图片/附件）、当天摘要、跨日汇总及每天一份独立恢复快照；每日快照仅会在网页或 Android App 处于使用状态时触发，完成当天首次同步后创建，并保留 14 天。草稿继续只留在当前设备，草稿箱内选择一条时会粘贴到当前输入框并移除原草稿。每次数据保存还会在本机 IndexedDB（浏览器数据库）保留最近 3 个自动恢复点。模型 API Key 不进入浏览器持久化存储；登录会话会持续保留，直到主动退出、清除站点数据或同步服务撤销会话。账号窗口会记录注册、登录、退出和手动同步结果。
- 数据库表、更新时间触发器、陈旧版本拦截器和按用户隔离的 RLS（行级安全）规则在 `supabase/schema.sql` 中；其中 `journal_backups` 是每日云端快照表。修改后请在 Supabase SQL Editor 重新执行该文件；项目配置只使用 Publishable Key，项目管理 Key 与数据库密码不进入前端代码。
...
在 Supabase SQL Editor 执行一次 [`supabase/schema.sql`](./supabase/schema.sql)。该脚本会创建 `journal_tasks`、`journal_backups` 表、私有 `journal-attachments` Storage 桶及按账号隔离的访问规则。执行后，下一次登录同步会自动迁移新保存的附件，并在当天首次成功同步后创建每日云端备份；无需在前端填写额外密钥。
