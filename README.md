...
- 云端保存日记（包含已保存的图片/附件）、当天摘要、跨日汇总及每天一份独立恢复快照；每日快照仅会在网页或 Android App 处于使用状态时触发，完成当天首次同步后创建，并保留 14 天。草稿继续只留在当前设备，草稿箱内选择一条时会粘贴到当前输入框并移除原草稿。每次数据保存还会在本机 IndexedDB（浏览器数据库）保留最近 3 个自动恢复点。模型 API Key 不进入浏览器持久化存储；登录会话会在当前浏览器或 App 中保留最多 2 天，期间刷新、关闭后重开都可继续使用；主动退出、清除站点数据或到期后需要重新登录。账号窗口会记录注册、登录、退出和手动同步结果。
