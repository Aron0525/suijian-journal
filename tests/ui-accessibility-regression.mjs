...
assert.match(index, /<button id="import-button" class="quick-tool-button" type="button" aria-controls="import-input">导入日记<\/button>/);
...
assert.match(styles, /\.top-tools\s*\{[^}]*flex-wrap:\s*nowrap/s);
assert.match(styles, /\.quick-tools-grid/);
assert.match(styles, /\.ai-config-dialog textarea\s*\{[^}]*max-height:\s*92px/s);
