...
assert.match(serviceWorker, /suijian-pwa-v45/);
assert.match(index, /app\.js\?release=20260906-remote-entry-merge/);
...
assert.match(app, /sw\.js\?release=20260906-remote-entry-merge/);
...
assert.match(topTools, /search-panel-button[\s\S]*tool-panel-button[\s\S]*model-config-button[\s\S]*cloud-sync-button[\s\S]*cloud-account-button/);
...
assert.equal((topTools.match(/class="top-tool-divider"/g) ?? []).length, 4, 'compact toolbar should use four vertical separators');
...
assert.match(mobileVersion, /"versionCode": 5/);
assert.match(mobileVersion, /"versionName": "1\.1\.3"/);
