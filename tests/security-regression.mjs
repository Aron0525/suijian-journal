...
assert.match(app, /if \(applyImmediately\) \{\s*await updater\.reload\(\);/);
assert.match(app, /checkNativeAppUpdate\(\{ quiet: true, applyImmediately: true \}\)/);
assert.match(app, /updater\.notifyAppReady\(\)/);
assert.match(app, /void initializeNativeUpdates\(\)/);
assert.match(app, /const NATIVE_APP_UPDATE_MANIFEST_URL = 'https:\/\/933647\.xyz\/native-app-update\.json'/);
...
assert.match(serviceWorker, /suijian-pwa-v53/);
assert.match(index, /app\.js\?release=20260910-admin-export-updater-v4/);
assert.match(index, /admin-export\.js\?release=20260910-admin-export-updater-v4/);
...
assert.match(app, /sw\.js\?release=20260910-admin-export-updater-v4/);
assert.match(index, /connect-src 'self' https:\/\/\*\.supabase\.co https:\/\/933647\.xyz https:\/\/aron0525\.github\.io/, 'the native WebView must be allowed to fetch both current and legacy update endpoints');
...
const archiveMarkup = index.match(/<section id="archive-page" class="home-page home-page-archive"[\s\S]+?<section class="archive-section"[\s\S]+?<\/section>\n\s*<\/section>/)?.[0] ?? '';
...
assert.match(topTools, /search-panel-button[\s\S]*tool-panel-button[\s\S]*model-config-button[\s\S]*cloud-sync-button[\s\S]*admin-shortcut-button[\s\S]*cloud-account-button/);
...
assert.match(topTools, /id="tool-panel-button" class="top-text-action"[^>]*>工具<\/button>/);
assert.match(topTools, /id="model-config-button" class="top-text-action"[^>]*>模型<\/button>/);
assert.match(topTools, /id="cloud-sync-button" class="top-text-action top-sync-action"[^>]*>同步<\/button>/);
assert.match(topTools, /id="cloud-account-button" class="top-text-action account-action"/);
assert.equal((topTools.match(/class="top-tool-divider"/g) ?? []).length, 5, 'administrator sessions add one conditional toolbar separator');
assert.match(index, /id="quick-tools-dialog"/);
assert.match(index, /id="summary-panel-button" class="quick-tool-button"/);
assert.match(index, /id="review-panel-button" class="quick-tool-button"/);
assert.match(index, /id="export-button" class="quick-tool-button"/);
assert.match(index, /id="import-button" class="quick-tool-button" type="button" aria-controls="import-input">导入日记<\/button>/);
assert.match(index, /id="import-input" type="file" accept="application\/json,\.json" hidden/);
assert.match(index, /id="backup-panel-button" class="quick-tool-button"/);
...
assert.match(mobileVersion, /"versionCode": 8/);
assert.match(mobileVersion, /"versionName": "1\.1\.6"/);
...
assert.match(app, /const legacyEntryColumns = 'id,entry_date,title,content,original_content,created_at,updated_at,deleted_at'/);
assert.match(app, /function isMissingCloudAttachmentsColumn\(error\)/);
assert.match(app, /state\.cloud\.attachmentsSupported = false/);
assert.match(app, /if \(state\.cloud\.attachmentsSupported !== false\) payload\.attachments = cloudAttachmentPayload\(entry\);/);
assert.match(app, /云端日记表尚未迁移附件字段，已启用文本同步兼容模式/);
